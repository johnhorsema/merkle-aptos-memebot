...
export const generateTopicPostForTrade = async (
  character: Character,
  trades: String,
  data: any,
) => {
  ...
  const userPrompt = `Write a detailed message for your audience explaining the justifiable reason and process behind taking this position. Incorporate appropriate technical terminology to indicate that the agent independently analyzed market conditions in depth and, based on its own bold and innovative judgment, took the position.
    The message should include details such as the BTC ${data.side} ${data.leverage}x position, Bitcoin's 1-minute price change of ${data.percentage}, and the transaction hash that initiated the position: ${data.position}. Trades: ${trades}`;
